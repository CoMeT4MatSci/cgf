...
cof = read('../test-data/COF-5_opt_SW.gen', index=-1)
remove_hatoms(cof)
...
cg_atoms = Atoms(['Y'] * len(r_c), positions=r_c, cell=cof.cell, pbc=True) # create coarse-grained representation based on core centers
...
plt.savefig('../test-data/COF-5_opt_SW_cg.pdf')
####
...
dyn = BFGS(cg_atoms, trajectory='SWopt.traj')
dyn.run(fmax=0.01)
